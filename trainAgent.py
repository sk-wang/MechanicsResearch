...
for i in range(99999):
  ...
  nowgame = nowgame.replace('{width}',str(rules[45]))
  nowgame = nowgame.replace('{brith}',str(rules[46]))
  nowgame = nowgame.replace('{dead}',str(rules[47]))
  nowgame = nowgame.replace('{density}',str(rules[48]))
  nowgame = nowgame.replace('{time}',str(rules[49]))
  nowgame = nowgame.replace('{connect}',str(rules[57]))
  netStr = '['
  for weigh in net.params:
    netStr = netStr + str(weigh) + ','
  netStr = netStr[0:len(netStr)-2] + ']'
  nowgame = nowgame.replace('{net}',netStr)
  g = open(os.path.dirname(os.path.realpath(__file__))+"/"+str(int(time.time())) + 'Generation'+str(i) + 'Eva' + str(best)+'.py', 'w+')
  print >> g,nowgame
  print "written"
  g.close()